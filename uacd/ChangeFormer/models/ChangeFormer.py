...
class MCDropout2d(nn.Dropout2d):
    """
    Custom MCDropout layer (2D version, for feature maps)
    It will override the 'forward' method and force the 'training' parameter to be True.
    Thus, it remains enabled in the model.eval() mode.
    """
...
class ChangeFormerV6(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256):
        ...
        self.TDec_x2 = DecoderTransformer_v3(input_transform='multiple_select', in_index=[0, 1, 2, 3],
                                             ...
                                             output_nc=output_nc,
                                             decoder_softmax=decoder_softmax, feature_strides=[2, 4, 8, 16])
        self.drop = MCDropout2d(p=0.1)

    def forward(self, x1, x2):
        # 1. Encoder extract basic features (List[Tensor])
        fx1_base = self.Tenc_x2(x1)
        fx2_base = self.Tenc_x2(x2)
        
        out_samples = []
        
        # self.drop must be defined in __init__, for example, self.drop = nn.Dropout2d(p=0.5)
        # Here, the loop is used to calculate the uncertainty (Entropy Map)
        for _ in range(6):
                # Apply Dropout to the feature list
            fx1_dropped = [self.drop(f) for f in fx1_base]
            fx2_dropped = [self.drop(f) for f in fx2_base]

                # Decoder
            cp_list = self.TDec_x2(fx1_dropped, fx2_dropped)

            final_cp = cp_list[-1]
            out_samples.append(final_cp) 
        ...
        outscp = torch.stack(out_samples, dim=0)
        
        probs = F.softmax(outscp, dim=2) 

        mean_probs = probs.mean(dim=0) 
        
        eps = 1e-8

        entropy_map = - (mean_probs * torch.log(mean_probs + eps)).sum(dim=1, keepdim=True)
   
        # return cp
        return cp, entropy_map