...
from models.losses import get_alpha, softmax_helper, FocalLoss, mIoULoss, mmIoULoss, cross_entropyu
...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        elif args.loss == 'ce+uloss':
            self._pxl_loss = cross_entropyu( ).to(self.device)
        elif args.loss == 'uloss':
            self._pxl_loss = cross_entropyu( ).to(self.device)
        elif args.loss == 'u':
            self._pxl_loss = cross_entropyu( ).to(self.device)
        else:
    ...
    def _load_checkpoint(self, ckpt_name='last_ckpt.pt'):
        ...
        if os.path.exists(os.path.join(self.checkpoint_dir, ckpt_name)):
            ...
            # 1. Load the file (add weights_only=False)
            pretrained_dict = torch.load(self.args.pretrain, map_location=self.device, weights_only=False)
            
            # 2. Intelligent weight extraction (Key step!)
            # If best_ckpt.pt is loaded, the weights are actually hidden in the key 'model_G_state_dict'
            if 'model_G_state_dict' in pretrained_dict:
                print("Detected full checkpoint, extracting 'model_G_state_dict'...")
                pretrained_dict = pretrained_dict['model_G_state_dict']
            self.net_G.load_state_dict(pretrained_dict, strict=False)
            self.net_G.to(self.device)
            self.net_G.eval()
        else:
    ...
    def _forward_pass(self, batch):
        ...
        elif self.args.loss == 'ce+uloss':
            pred_list, entropy_map = self.G_pred
            self.G_final_pred = pred_list[-1]
        elif self.args.loss == 'uloss':
            pred_list, entropy_map = self.G_pred
            self.G_final_pred = pred_list[-1]
        elif self.args.loss == 'u':
            pred_list, entropy_map = self.G_pred
            self.G_final_pred = pred_list[-1]
        else:
    ...
    def _backward_G(self):
        ...
        if self.multi_scale_train == "True":
            ...
            pred_list, entropy_map = self.G_pred
            self.G_loss = self._pxl_loss(pred_list[-1], gt, entropy_map)
        elif self.args.loss == 'uloss':
