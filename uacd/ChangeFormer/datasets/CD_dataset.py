...
def get_img_post_path(root_dir,img_name, split=None):
    if split is None:
...
def get_img_path(root_dir, img_name, split=None):
    if split is None:
...
def get_label_path(root_dir, img_name, split=None):
    if split is None:
...
class CDDataset(ImageDataset):
    ...
    def __init__(self, root_dir, img_size, split='train', is_train=True, label_transform=None,
                 to_tensor=True, label_suffix=None):
    ...
    def __getitem__(self, index):
        name = self.img_name_list[index]
        A_path = get_img_path(self.root_dir, self.img_name_list[index % self.A_size], split=self.split)
        B_path = get_img_post_path(self.root_dir, self.img_name_list[index % self.A_size], split=self.split)
        img = np.asarray(Image.open(A_path).convert('RGB'))
        img_B = np.asarray(Image.open(B_path).convert('RGB'))
        L_path = get_label_path(self.root_dir, self.img_name_list[index % self.A_size], split=self.split)
        
        try:
            label = np.array(Image.open(L_path).convert("L"), dtype=np.uint8)
        except:
            label = np.array(Image.open(L_path.replace("tif", "png")).convert("L"), dtype=np.uint8)

        # if you are getting error because of dim mismatch ad [:,:,0] at the end

        #  In the binary classification, the foreground is marked as 255
        if np.max(label) > 1:
        # if self.label_transform == 'norm':
            label = label // 255
        
        # print("label-max={}".format(np.max(label)))
        # print("A-max={}".format(np.max(img)))
        # print("self.labbel_transform = {}".format(self.label_transform))
