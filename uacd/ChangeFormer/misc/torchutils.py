...
def visualize_imgs(*imgs):
    """
    Visual images, images in ndarray format
...
def norm_tensor(tensor,min_=None,max_=None, mode='minmax'):
    ...
    Output: Normalized image with the same size as the original in H*W dimension
    """
    assert tensor.ndim >= 2
    shape = tensor.shape
    tensor = tensor.view([*shape[:-2], shape[-1]*shape[-2]])
...
def visulize_features(features, normalize=False):
    """
    Visualize feature maps, make grids of each dimension together
    """
...
def visualize_tensors(*tensors):
    """
    Visualize tensors, supporting single-channel features or 3-channel images
...
def seed_torch(seed=2019):
    ...
    # Keep the following random seeds, data input, random expansion, etc. consistent
    random.seed(seed)
    os.environ['PYTHONHASHSEED'] = str(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    torch.cuda.manual_seed(seed)
    # After adding all the random seeds and updating the model, the intermediate results were still different.
    # The phenomenon of discovering this: The results of the first two rounds were still the same. The results will change as the model is updated.
...
def balanced_cross_entropy(input, target, weight=None,ignore_index=255):
    """
    The cross-entropy loss with category balance currently only supports two categories
    TODO: Extend to multi-class C>2
    """
    if target.dim() == 4:
        target = torch.squeeze(target, dim=1)
    if input.shape[-1] != target.shape[-1]:
    ...
    target_pos[target_pos!=1] = ignore_index  # Ignore the regions that are not positive samples
    target_neg = target.float()
    target_neg[target_neg!=0] = ignore_index  # Ignore the regions that are not negative samples
...
def mul_cls_acc(preds, targets, topk=(1,)):
    """Calculate the top-k accuracy of multi-label classification, topk-acc, topk-error=1-topk-acc;
First, calculate the average accuracy rate of each graph, and then calculate the average accuracy rate of all graphs
    :param pred: N * C
    :param target: N * C
    :param topk:
    :return:
    ...
    with torch.no_grad():
        ...
        correct = torch.zeros([bs, maxk]).long()  # Record the number of correctly predicted labels
        ...
        n = (targets == 1).long().sum(1)  # N*1, the number of targets contained in each graph
        # print(n)
        res = []
        for k in topk:
            acc_k = correct[:, :k].sum(1).float() / n.float()  # The average accuracy rate of each graph, the number of predicted correct targets/the total number of targets
...
def tensor2np(input_image, if_normalize=True):
    ...
    if image_numpy.ndim == 2:
        ...
        image_numpy = np.transpose(image_numpy, (1, 2, 0))
        #  If the input is a grayscale image with C==1, then output array, ndim==2;
        if C == 1:
