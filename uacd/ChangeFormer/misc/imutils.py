...
def cv_rotate(image, angle, borderValue):
    ...
    # The -angle position parameter is a negative value of the Angle parameter, indicating clockwise rotation. The 1.0 position parameter scale is for adjusting the size ratio (image scaling parameter), and it is recommended to be 0.75
...
def im2arr(img_path, mode=1, dtype=np.uint8):
    ...
    if mode==1:
        ...
        if arr.ndim == 3:
            a, b, c = arr.shape
            if a < b and a < c:  # When arr is C*H*W, the channel sequence needs to be swapped
