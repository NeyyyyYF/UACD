...
from datasets.data_utils2 import CDDataAugmentation
num_classes = 1
...
def read_RSimages(mode, read_list=False):
    ...
    # if mode=='train':
    #     random_sum = int(len(data_list) * 0.1)
    #     data_list = random.sample(data_list,random_sum)

    data_A, data_B, labels, filenames = [], [], [], []
    for idx, it in enumerate(data_list):
        # if (it[-4:]=='.png'):
        if (it[-4:] in ('.png','.tif')):
            img_A_path = os.path.join(img_A_dir, it)
            img_B_path = os.path.join(img_B_dir, it)
            # label_path = os.path.join(label_dir, it.replace('.tif', '.png'))
    ...
    return data_A, data_B, labels, filenames
...
class RS(data.Dataset):
    def __init__(self, mode, random_crop=False, crop_nums=6, sliding_crop=False, crop_size=512, random_flip=False):
        ...
        data_A, data_B, labels, filenames = read_RSimages(mode, read_list=False)
        if sliding_crop:
    ...
    def __getitem__(self, idx):
        ...
        return F.to_tensor(data_A), F.to_tensor(data_B), label, filename
