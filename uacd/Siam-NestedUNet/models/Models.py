...
class MCDropout2d(nn.Dropout2d):
    ...
    It will override the 'forward' method, forcing the 'training' parameter to be True,
    thus remaining active even in model.eval() mode.
    """
...
class SNUNet_ECAMp2X(nn.Module):
    def __init__(self, in_ch=3, out_ch=2):
        super(SNUNet_ECAMp2, self).__init__()
        torch.nn.Module.dump_patches = True
        n1 = 32     
        filters = [n1, n1 * 2, n1 * 4, n1 * 8, n1 * 16]
        ...
        self.conv1_1 = conv_block_nested(filters[1] * 2 + filters[2], filters[1], filters[1])
        self.conv2_1 = conv_block_nested(filters[2] * 2 + filters[3], filters[2], filters[2])
        self.conv3_1 = conv_block_nested(filters[3] * 2 + filters[4] * 2, filters[3], filters[3])
    ...
    def _forward_impl(self, xA, xB):
        ...
        x1_0A = self.conv1_0(self.pool(x0_0A))
        x1_0B = self.conv1_0(self.pool(x0_0B))
        x1_0A, x1_0B = self.drop(x1_0A), self.drop(x1_0B) # Dropout

        # --- Level 2 ---
        x2_0A = self.conv2_0(self.pool(x1_0A))
        x2_0B = self.conv2_0(self.pool(x1_0B))
        x2_0A, x2_0B = self.drop(x2_0A), self.drop(x2_0B) # Dropout

        # --- Level 3 ---
        x3_0A = self.conv3_0(self.pool(x2_0A))
        x3_0B = self.conv3_0(self.pool(x2_0B))
        x3_0A, x3_0B = self.drop(x3_0A), self.drop(x3_0B) # Dropout

        x4_0A = self.conv4_0(self.pool(x3_0A))
        x4_0B = self.conv4_0(self.pool(x3_0B))
        x4_0A, x4_0B = self.drop(x4_0A), self.drop(x4_0B) # Dropout
        ...
        x1_2 = self.conv1_2(torch.cat([x1_0A, x1_0B, x1_1, self.Up2_1(x2_1)], 1))
        x0_3 = self.conv0_3(torch.cat([x0_0A, x0_0B, x0_1, x0_2, self.Up1_2(x1_2)], 1))
        x3_1 = self.conv3_1(torch.cat([x3_0A, x3_0B, self.Up4_0(x4_0A), self.Up4_0(x4_0B)], 1))
    ...
    def forward(self, xA, xB):
        out_samples = []
        for _ in range(2):
            out_samples.append(self._forward_impl(xA, xB))
        outs = torch.stack(out_samples, dim=0)
        probs = F.softmax(outs, dim=2) 
        mean_probs = probs.mean(dim=0)  # [B, 2, H, W]
        
        eps = 1e-8
        entropy_map = - (mean_probs * torch.log(mean_probs + eps)).sum(dim=1, keepdim=True) # [B, 1, H, W]

        final_out = outs.mean(dim=0) 
        
        return final_out, entropy_map

class SNUNet_ECAMp2(nn.Module):
    # SNUNet-CD with ECAM
    def __init__(self, in_ch=3, out_ch=2):
        super(SNUNet_ECAMp2, self).__init__()
        torch.nn.Module.dump_patches = True
    ...
    def forward(self, xA, xB):
        ...
        for _ in range(6):
            ...
            x2_0A = self.conv2_0(self.pool(x1_0A))
            x3_0A = self.conv3_0(self.pool(x2_0A))
            x1_0A = self.drop(x1_0A)
            x2_0A = self.drop(x2_0A)
            x3_0A = self.drop(x3_0A)
        # x4_0A = self.conv4_0(self.pool(x3_0A))
            '''xB'''
            x1_0B = self.conv1_0(self.pool(x0_0B))
            x2_0B = self.conv2_0(self.pool(x1_0B))
            x3_0B = self.conv3_0(self.pool(x2_0B))
            x4_0B = self.conv4_0(self.pool(x3_0B))
            x1_0B = self.drop(x1_0B)
            x2_0B = self.drop(x2_0B)
            x3_0B = self.drop(x3_0B)
            x4_0B = self.drop(x4_0B)
        ...
        outs1 = torch.stack(out_samples, dim=0)
        outs = outs1[0:6]
        out = outs1[-1]
        ...
        return (out, entropy_map)





class SNUNet_ECAMp1(nn.Module):
    # SNUNet-CD with ECAM
    def __init__(self, in_ch=3, out_ch=2):
        super(SNUNet_ECAM, self).__init__()
        torch.nn.Module.dump_patches = True
    ...
    def forward(self, xA, xB):
        ...
        out_samples = []
        for _ in range(5):
            out_samples.append(self.conv_final(out))
            # Stacking result [5, B, 1, H, W]
        outs = torch.stack(out_samples, dim=0)
        ...
        eps = 1e-8
        entropy_map = - (mean_probs * (mean_probs + eps).log()).sum(dim=1, keepdim=True)  # [B, 1, H, W]
        # print(entropy_map.shape)torch.Size([16, 1, 256, 256])

        out = self.conv_final(out)
        # print(out.shape)torch.Size([16, 2, 256, 256])


        return (out, entropy_map)

        
class SNUNet_ECAMp3(nn.Module):
    # SNUNet-CD with ECAM
    def __init__(self, in_ch=3, out_ch=2):
        super(SNUNet_ECAMp3, self).__init__()
        torch.nn.Module.dump_patches = True
        n1 = 32     # the initial number of channels of feature map
        filters = [n1, n1 * 2, n1 * 4, n1 * 8, n1 * 16]
    ...
    def forward(self, xA, xB):
        ...
        out_samples = []
        for _ in range(5):
            out_samples.append(self.conv_final(self.ca(self.drop(out)) * (self.drop(out) + self.ca1(self.drop(intra)).repeat(1, 4, 1, 1))))
            # Stacking result [5, B, 1, H, W]
        outs = torch.stack(out_samples, dim=0)
        ...
        eps = 1e-8
        entropy_map = - (mean_probs * (mean_probs + eps).log()).sum(dim=1, keepdim=True)  # [B, 1, H, W]
        out = self.conv_final(self.ca(self.drop(out)) * (self.drop(out) + self.ca1(self.drop(intra)).repeat(1, 4, 1, 1)))
        return (out, entropy_map)