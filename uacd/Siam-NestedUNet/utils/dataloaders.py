...
def cdd_loader(img_path, label_path, aug):
    ...
    label_array = np.clip(label_array * 255, 0, 255)
    label = Image.fromarray(label_array.astype(np.uint8))
...
class CDDloader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        filename = self.full_load[index]['name']
        # return self.loader(img_path,
        #                    label_path,
        #                    filename,
        #                    self.aug)
                # cdd_loader return three values: img1, img2, label
        img1, img2, label = self.loader(img_path,
                                        label_path,
                                        self.aug)
        
        return img1, img2, label, filename  # return four values
